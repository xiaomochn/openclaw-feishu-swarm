...
import { notifySent } from "./bot-registry/notify.js";
...
export async function sendMessageFeishu(
  ...
  if (replyToMessageId) {
    ...
    const replyResult = toFeishuSendResult(response, receiveId);
    notifySent({
      cfg,
      accountId,
      to: receiveId,
      ...
      content: rawText,
      contentType: "post",
      mentions: mentions?.map((m) => m.openId),
    });
    return replyResult;
  }
...
export async function sendCardFeishu(params: SendFeishuCardParams): Promise<FeishuSendResult> {
  ...
  if (replyToMessageId) {
    ...
    const cardReplyResult = toFeishuSendResult(response, receiveId);
    notifySent({
      cfg,
      accountId,
      to: receiveId,
      chatId: receiveId,
      messageId: cardReplyResult.messageId,
      replyToMessageId,
      content: "[card]",
    });
    return cardReplyResult;
  }
  ...
  const cardSendResult = toFeishuSendResult(response, receiveId);
  notifySent({
    cfg,
    accountId,
    to: receiveId,
    chatId: receiveId,
    messageId: cardSendResult.messageId,
    replyToMessageId,
    content: "[card]",
  });
